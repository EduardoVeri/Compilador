        ...
        if(codigoIntermediario[i] != NULL){
            ...
            if(codigoIntermediario[i]->arg3 != NULL){
                if(codigoIntermediario[i]->arg3->tipo == IntConst){
                    if(codigoIntermediario[i]->arg1->boolReg == 1)
                        printf("$t%d\n", codigoIntermediario[i]->arg1->val);
                    else
                        printf("%d\n", codigoIntermediario[i]->arg1->val);
                }
...
void codIntExpOpRel(PONTEIRONO arvoreSintatica, PONTEIROITEM tabelaHash[]){
    ...
    criarCodigoIntermediario(arvoreSintatica->filho[0], tabelaHash);
    aux = numReg - 1;
    instrucaoOp->arg1 = criaEndereco(IntConst, aux, NULL, 1);
...
void codIntExpAtrib(PONTEIRONO arvoreSintatica, PONTEIROITEM tabelaHash[]){
    ...
    instrucaoStore->arg2 = criaEndereco(IntConst, instrucaoAtrib->arg1->val, NULL, 0);
