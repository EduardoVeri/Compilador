...
void codIntExpAtrib(PONTEIRONO arvoreSintatica, PONTEIROITEM tabelaHash[]){
    ...
    criarCodigoIntermediario(arvoreSintatica->filho[0], tabelaHash);
    instrucaoAtrib->arg1 = criaEndereco(IntConst, numReg - 1, NULL);

    criarCodigoIntermediario(arvoreSintatica->filho[1], tabelaHash);
    instrucaoAtrib->arg2 = criaEndereco(IntConst, numReg - 1, NULL);

    codigoIntermediario[indiceVetor] = instrucaoAtrib;
    indiceVetor++;

    instrucaoStore = criaInstrucao("STORE");
    instrucaoStore->arg1 = criaEndereco(String, 0, arvoreSintatica->filho[0]->lexema);
    instrucaoStore->arg2 = criaEndereco(IntConst, instrucaoAtrib->arg1->val, NULL);
    instrucaoStore->arg3 = criaEndereco(Vazio, 0, NULL);
